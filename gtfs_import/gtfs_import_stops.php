...
$mysql = new MySQL();
if($_GET['delete'] == "true")
    $mysql->query("Truncate gtfs_stops");
$datei = fopen("data/stops.txt","r");
$firstrow = 1;
$rowcount = 0;
$insertcloums;
while(!feof($datei)){
	...
	if($firstrow == 0){
		$row = explode(",",$row);
        $query = "insert into gtfs_stops (".$insertcloums.") values (";
        foreach($row as $data){
            //überflüssige Texte rausschemißen Region Friesland
